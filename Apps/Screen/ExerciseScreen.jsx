...
export default function ExerciseScreen() {
  ...
  const fetchExercises = async (pregnancyStage) => {
    try {
      ...
      let exerciseQuery;
      if (pregnancyStage) {
        exerciseQuery = query(exercisesRef, where('pregnancyStage', '==', pregnancyStage));
      } else {
      ...
      <Text style={tw`text-xl font-semibold mt-5 text-pink-600`}>Recommended for you</Text>
        ...
        {exercises.length > 0 && (
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate('ExerciseDetail', { exerciseId: exercises[0].id })}
          >
            <Image source={{ uri: exercises[0].imageUrl }} style={tw`w-full h-48 rounded-xl`} />
            <View style={tw`mt-4`}>
              <Text style={tw`text-sm text-gray-500`}>Let’s Continue, Where you left off!!</Text>
              <Text style={tw`text-lg font-bold`}>{exercises[0].exerciseName}</Text>
            </View>
